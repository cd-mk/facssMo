$(function () {
  ...
  if (!$.app.menu.is_touch_device()) {
    ...
    if (userChatsAll.length > 0) {
      var chatsUserAll = new PerfectScrollbar(userChatsAll[0], {
        wheelPropagation: false
      });
    }
...
function enterChat(source) {
  ...
  if (/\S/.test(message)) {
    ...
    $('.user-chats .chat:last-child .chat-body').append(html);
    $('.message').val('');
...
function enterChatAll(source) {
  ...
  if (/\S/.test(messageAll)) {
    ...
    $('.user-chats-all .chat:last-child .chat-body').append(html);
    $('.message-all').val('');
